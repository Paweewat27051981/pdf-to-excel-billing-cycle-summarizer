...
export interface Branch {
  ...
  // true = ผู้ดูแลราคา (admin) — คนเดียวที่แก้/นำเข้า Master ราคาขนส่ง + ราคาเฉพาะรอบได้
  // ทำงานได้เหมือน HQ ทุกอย่าง แต่ไม่โผล่ในดรอปดาวน์ "เลือกสาขา" (ไม่ใช่สาขาจริง)
  canEditRates?: boolean;
  isSystemUser?: boolean; // true = ผู้ใช้ระบบ (ไม่ใช่สาขา) -> ซ่อนจากรายการสาขาทั่วไป
...
export interface RateMasterHistory {
  ...
  changedAt: string;
  changeReason: string;
  // ---- ราคาเฉพาะรอบ (ถ้ามี cycleId = แถวนี้คือประวัติของ "ราคาเฉพาะรอบ" ไม่ใช่ราคาหลัก) ----
  // เดิมบันทึกเฉพาะราคาหลัก แต่ราคาที่ "คิดเงินจริง" คือราคาเฉพาะรอบ
  // -> แก้/นำเข้าราคาเฉพาะรอบแล้วไม่มีร่องรอย ตามหลังไม่ได้ว่าใครแก้เมื่อไหร่ (เจ้าของสั่งเพิ่ม 9 ก.ย.69)
  // ใช้ตารางเดียวกับราคาหลักเพื่อให้หน้า "บันทึกการแก้ไข" แสดงรวมกันได้เลย
  cycleId?: string;      // งวดที่ราคานี้มีผล (ว่าง = ราคาหลัก)
  cycleName?: string;    // ชื่องวด ณ เวลาที่แก้ (เก็บไว้กันงวดถูกลบแล้วอ่านไม่ออก)
  branchId?: string;     // สาขา (ราคาหลักดูจาก rateMaster ได้ แต่ override ควรเก็บตรงๆ)
}
